...
class CBBAnalyticsScraper:
    ...
    def clean_percentile_values(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Examples:
        - "98129.3" -> 129.3 (percentile 98 + ORtg 129.3)
        - "10061.8%" -> "61.8%" (percentile 100 + eFG% 61.8%)
        - "100+40.4" -> "+40.4" (percentile 100 + Net Rtg +40.4)
        """
        import re
        ...
        for col in df.columns:
            ...
            # Don't skip columns just because they have "unnamed" - those might have real data
            if any(x in col_str for x in ['team', ' gp']):  # Note: space before gp to avoid false matches
                skip_columns.add(col)
            # Also skip columns that are just positional markers (Unnamed: 0, Unnamed: 1, etc.)
            if col_str.startswith('unnamed') and 'rtg' not in col_str and '%' not in col_str and 'rate' not in col_str:
                skip_columns.add(col)
        ...
        for col in df.columns:
            if col in skip_columns:
                continue
            try:
                def split_percentile_value(val):
                    ...
                    # Pattern 1: "100+40.4" -> "+40.4" (positive values with + sign)
                    ...
                    # Pattern 2: "100-40.4" -> "-40.4" (negative values with - sign)
                    ...
                    # Pattern 3: "10061.8%" -> "61.8%" (3-digit percentile + percentage value)
                    ...
                    # Pattern 4: "9960.5%" -> "60.5%" (2-digit percentile + percentage value)
                    ...
                    # Pattern 4b: "645.0%" -> "45.0%" (1-digit percentile + percentage value)
                    ...
                    # Pattern 5: "98129.3" -> "129.3" (2-digit percentile + 3-digit rating)
                    ...
                    # Pattern 6: "1399.4" -> "99.4" (1 or 2 digit percentile + 2-digit rating)
                    ...
                    # Pattern 7: "20101.8" -> "101.8" (2-digit percentile + 3-digit rating, when pattern 5 didn't match because of float conversion)
                    match = re.match(r'^(\d{2})(\d{2,3}\.\d+)$', val_str)
                    if match:
                        percentile = int(match.group(1))
                        actual_value = match.group(2)
                        if 0 <= percentile <= 100:
                            # Check if remaining value is reasonable
                            try:
                ...
                df[col] = df[col].apply(split_percentile_value)
